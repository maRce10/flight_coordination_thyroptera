...
ani.options(ani.res = 80, ani.width = 480 * 0.7, ani.height = 300 * 0.7)
...
gifs <- function(lng_dat, z, type = "gif", step = 90){ 
  ...
  s3d <- scatterplot3d(X[, c("x", "y", "z")], pch = "", grid=FALSE, box=FALSE, xlim = range(lng_dat$x), ylim = range(lng_dat$y), zlim = range(lng_dat$z), tick.marks = FALSE, xlab = "", ylab = "", zlab = "", main = paste(format(round(end / 120, 2), nsmall = 2), "s"), mar = c(0, 0, 1.5, 0), sub = paste("Excel row:", X$xl.row[end]))
